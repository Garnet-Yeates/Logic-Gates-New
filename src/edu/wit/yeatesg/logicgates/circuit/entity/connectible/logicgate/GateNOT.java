...
public class GateNOT extends ConnectibleEntity implements Rotatable {
    ...
    @Override
    public void draw(GraphicsContext gc, Color col, double opacity) {
        gc.setStroke(col == null ? Color.BLACK : col);
        gc.setLineWidth(getLineWidth());
        ConnectionNode.drawNegationCircle(gc, col, drawPoints.get(1), size == NORMAL ? 1 : 0.8);
        ...
        gc.strokeLine(triBot.x, triBot.y, triTL.x, triTL.y);
        gc.strokeLine(triTL.x, triTL.y, triTR.x, triTR.y);
        gc.strokeLine(triTR.x, triTR.y, triBot.x, triBot.y);
        out.draw(gc, col, opacity);
        in.draw(gc, col, opacity);
    }
    ...
    public boolean canTheoreticalWireGoHere(Wire.TheoreticalWire theo, PermitList permits, boolean strictWithWires) {
    ...
    public void onPropertyChangeViaTable(String propName, String old, String newVal) {
        if (isItemEntity()) {
            onPropertyChangeViaOperation(propName, old, newVal);
            treeItem.onClick();
        }
        else
